...
> exists, all seven gates pass, and **two certificates have been issued — one of them
> committed to this repository at [`certificates/`](certificates/)** — see
...
| # | Issued (UTC) | Source SHA-256 | Source | Spec | Rev | Verdict | Gates G1–G7 | Ready |
|---|---|---|---|---|---|---|---|---|
| **2** | 2026-08-25T10:12:50Z | `465297171ccbf63f1f7f71ef0ccdec9f9a72dccc254673b7250706a624086b85` | **SYNTHETIC** · 86,928 B · 168 traces · 12×14×64 | v1.0 | 1 | **EQUIVALENT** | `PPPPPPP` | **`true`** |

**Row 2 — the reference certificate, and the first one committed to the repository.**

Issued at commit `20e5a7a` from a clean tree, against a fixture **generated by committed
code with a fixed seed**. Unlike row 1, whose source is restricted, **this certificate and
its source are both reproducible by anyone**: the JSON is in
[`certificates/`](certificates/) and the fixture is three lines of Python.

**It exists because an external reviewer opened `certificates/` and found an empty room.**
The project's entire thesis is *the file plus the proof* — and it shipped the engine, the
schema, this ledger and the philosophy, with no proof a stranger could open. That was a
fair hit and this is the answer to it.
