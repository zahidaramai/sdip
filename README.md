...
|---|---|---|---|---|
| `G1` | Byte coverage `== {1..240}`, itemsize `== 240`, zero overlaps, zero void gaps | Any uncovered or doubly-covered byte | F1 | not built |
...
sdip spec build   # generate + validate a gap-free spec; runs G1          [F1]
...
| **Gates enforcing** | **0 of 7.** See [The Gates](#-the-gates). |
| **Certificates issued** | **0** |
...
- SEG-Y (rev 0, 1, 2, 2.1) → MDIO v1 / Zarr v3 ingestion
